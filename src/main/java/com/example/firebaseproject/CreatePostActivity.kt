...
class CreatePostActivity : AppCompatActivity() {
    ...
    private fun addItem() {
        ...
        if (name.isEmpty()) {
            Snackbar.make(editProductName, "상품 이름을 입력하세요!", Snackbar.LENGTH_SHORT).show()
            return
        }
        val price = editPrice.text.toString()
        if (price.isEmpty()) {
            Snackbar.make(editPrice, "가격을 입력하세요!", Snackbar.LENGTH_SHORT).show()
            return
        }
        val desc = editDesc.text.toString()
        if (desc.isEmpty()) {
            Snackbar.make(editDesc, "상품 설명을 입력하세요!", Snackbar.LENGTH_SHORT).show()
            return
        }
